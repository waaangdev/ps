...
for i in range(n):
    ...
    for j in range(n):
        if(m[i][j] == 1):
            q = deque([[i,j,0,0,1,0],[i,j,1,0,1,0],[i,j,2,0,1,0]])
        ...
        for j in range(4):
            for k in range(1*(qpop[2] < 2),n+1+(-(n+1)+2)*(qpop[2] == 2)):
                ...
                if(dum1 > -1 and dum1 < n and dum2 > -1 and dum2 < n):
                    dum = bang[qpop[3]][qpop[2]][dum1][dum2]
                    if(dum == -1 or (dum[0] == t and dum[1] > qpop[5])):
                        bang[qpop[3]][qpop[2]][dum1][dum2] = [t,qpop[5]]
                        if(m[dum1][dum2]-1 == qpop[3]+1):
                            q.append([dum1,dum2,qpop[2],qpop[3]+1,0,qpop[5]])
                            
                            if(qpop[3]+1 == n*n-1):
                                br= 1
                                min_ = min(min_,qpop[5])
                        else:
                            q.append([dum1,dum2,qpop[2],qpop[3],0,qpop[5]])
        if(qpop[4] == 0):
            for j in range(3):
                if(j != qpop[2]):
                    if(bang[qpop[3]][j][qpop[0]][qpop[1]] == -1):
                        q.append([qpop[0],qpop[1],j,qpop[3],1,qpop[5]+1])
    if(br):
        break
print(t,min_)